...
export class AIService {
    ...
    private async callOpenAI(model: string, prompt: string, maxTokens: number, temperature: number): Promise<AICompletionResponse> {
        const response = await fetch(this.apiEndpoint || 'https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You are a code completion assistant. Provide concise, accurate code completions based on the context.'
                    },
                    {
                        role: 'user',
                        content: prompt
    ...
    private async callAnthropic(model: string, prompt: string, maxTokens: number, temperature: number): Promise<AICompletionResponse> {
    ...
    private async callAzure(model: string, prompt: string, maxTokens: number, temperature: number): Promise<AICompletionResponse> {
    ...
    private async callOllama(model: string, prompt: string, maxTokens: number, temperature: number): Promise<AICompletionResponse> {
