...
export class SmartImportFeature {
    ...
    constructor() {
        this.diagnosticCollection = vscode.languages.createDiagnosticCollection('code-slash-smart-import');
    ...
    private extractUndefinedSymbols(document: vscode.TextDocument): string[] {
        ...
        const definedSymbols: Set<string> = new Set([
            ...
            'clearInterval', 'Promise', 'Map', 'Set', 'WeakMap', 'WeakSet',
            'Object', 'Array', 'String', 'Number', 'Boolean', 'Function',
            'Symbol', 'Error', 'TypeError', 'SyntaxError', 'ReferenceError',
        ...
        while ((match = identifierPattern.exec(text)) !== null) {
            ...
            if (!definedSymbols.has(symbol) && !existingImports.has(symbol) && 
                !symbol.match(/^\d/) && !['true', 'false', 'null', 'undefined'].includes(symbol)) {
...
Only respond with symbols that have high confidence of being external imports. If unsure, don't include the symbol.`;
    ...
    private showImportSuggestions(candidates: ImportCandidate[], document: vscode.TextDocument): void {
    ...
    public async addAllImports(candidates: ImportCandidate[], document: vscode.TextDocument): Promise<void> {
        const edit = new vscode.WorkspaceEdit();
